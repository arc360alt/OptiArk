...
def update_local_version(file_path, new_version):
    """Update the localVersion in a mutils.json5 file."""
    try:
        # Read the file
        with open(file_path, 'r', encoding='utf-8') as f:
            content = f.read()
        
        # Pattern to match localVersion line (handles various spacing and quote styles)
        pattern = r'(localVersion\s*:\s*["\'])([^"\']+)(["\'])'
        
        # Check if pattern exists
        match = re.search(pattern, content)
        if not match:
            print(f"    Warning: localVersion pattern not found in {file_path}")
            return False
        
        # Replace the version using a lambda function to avoid regex group reference issues
        ...
        print(f"    Successfully updated localVersion to '{new_version}' in {os.path.basename(file_path)}")
        return True
...
def main():
    ...
    new_version = input("Enter the new version (e.g., 'Version 1.7.0'): ").strip()
    if not new_version:
        print("No version provided. Exiting.")
        return
    
    print(f"\nSearching for OptiArk folders in: {PYTHON_FILES_ROOT}")
    ...
    for folder in optiark_folders:
        ...
        for mutils_file in mutils_files:
            if update_local_version(mutils_file, new_version):
                total_updated += 1
